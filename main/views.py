...
# Login page ng student and faculty
...
def course_page(request, code):
    try:
        ...
        if is_student_authorised(request, code):
            try:
                announcements = Announcement.objects.filter(status=1,course_code=course)
                assignments = Assignment.objects.filter(status=1,course_code=course.code)
                materials = Material.objects.filter(status=1,course_code=course.code)
...
def deleteAssignment(request, code, id):
    if is_faculty_authorised(request, code):
        assignment = Assignment.objects.get(course_code=code, id=id)
        assignment.delete()
        messages.warning(request, 'Assignment is deleted')
        return redirect('/faculty/' + str(code))
    else:
        return redirect('std_login')

def editAssignment(request, code, id):
    if is_faculty_authorised(request, code):
        ...
        return render(request, 'main/update-assignment.html', context)
    else:
        return redirect('std_login')

def updateAssignment(request, code, id):
    if is_faculty_authorised(request, code):
        try:
            assignment = Assignment.objects.get(course_code_id=code, id=id)
            form = AssignmentForm(request.POST, None ,request.FILES or None, instance=assignment) 
            if form.is_valid():
                form.save()
...
def allAssignmentsSTD(request, code):
    if is_student_authorised(request, code):
        course = Course.objects.get(code=code)
        assignments = Assignment.objects.filter(status=1,course_code=course)
        context = {
            'assignments': assignments,
            'course': course,
            'student': Student.objects.get(student_id=request.session['student_id']),
...
def editCourseMaterial(request, code, id):
    if is_faculty_authorised(request, code):
        course_material = Material.objects.get(course_code_id=code, id=id)
        form = MaterialForm(instance=course_material)
        context = {
        ...
        return render(request, 'main/update-course-material.html', context)
    else:
        return redirect('std_login')

def updatecoursematerial(request, code, id):
    if is_faculty_authorised(request, code):
        try:
            course_material = Material.objects.get(course_code_id=code, id=id )
            form = MaterialForm(request.POST or None ,request.FILES or None, instance=course_material)
            if form.is_valid():
                form.save()
                messages.info(request, 'Course Material updated successfully.')
                return redirect('/faculty/' + str(code))
        except:
            return redirect('/faculty/' + str(code))
